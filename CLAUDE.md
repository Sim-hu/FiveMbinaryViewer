...
- **実装が終わるたびに main へプッシュする**。作業ブランチでの実装が完了し型チェックが通ったら、コミットして `main` に反映し `origin/main` へ push する。別ブランチで作業した場合は main にマージしてから push する。
- **更新ごとに `package.json` の `version` を 0.0.1 ずつ上げる**。何か変更を加えてコミットする際は patch を 1 上げる（例: 0.0.1 → 0.0.2）。バージョンは `vite.config.ts` の `define` で `__APP_VERSION__` として注入され、画面ヘッダー右上に `vX.Y.Z` で表示される。Web で動いているバージョンを識別するためなので上げ忘れない。
